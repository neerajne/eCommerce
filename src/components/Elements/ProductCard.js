    ...
    <div className="w-full sm:w-1/2 md:w-1/2 lg:w-1/3 xl:w-1/3 p-2">
      <div className="bg-white rounded-lg border border-gray-200 shadow-md dark:bg-gray-800 dark:border-gray-700">
        <Link to = {`/products/${id}`} className="relative block">
          {best_seller ? <span className="absolute top-4 left-2 px-2 bg-orange-500 bg-opacity-90 text-white rounded">Best Seller</span>  :<span className="absolute top-4 left-2 px-2 bg-orange-500 bg-opacity-90 text-white rounded"></span> }   
          <img className="rounded-t-lg w-full h-auto sm:h-64" src={poster} alt={name} />
        </Link>
        <div className="p-5">
          <Link to="/">
